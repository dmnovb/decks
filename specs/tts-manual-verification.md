...
1. Happy path playback
   ...
   - Confirm audio plays and the button shows a loading spinner while the request/audio is active.
   - If multiple voices are enabled, confirm the voice menu selection persists after refresh.
...
3. ElevenLabs 5xx/refund
   ...
   - Click a speaker button.
   - Confirm the API returns `502` (or `500` if the fetch throws) and the spent `tts_playback` credits are refunded with reason `tts_playback_failed`.
...
6. Abandoned playback
   - Click a speaker button and immediately advance to another card or end the session.
   - Confirm the in-flight playback is stopped or aborted and stale audio from the previous card does not play.

## Known limitations

- Credits are spent before the ElevenLabs request. If the process crashes or times out after the spend but before the refund path runs, manual reconciliation may be required.
- If ElevenLabs starts an audio stream and then fails after response headers are committed, the client may receive partial audio and the spend is kept.
- Middleware rate limiting is per IP, not per user. Credit balance remains the primary per-user cost backstop for v1.
