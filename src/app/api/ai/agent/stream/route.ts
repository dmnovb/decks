      ...
      async start(controller) {
        try {
          ...
          while (true) {
            ...
            sendEvent(controller, "function_start", { functions: toolUseBlocks.map((b) => b.name) });
            ...
            const toolResults: Anthropic.ToolResultBlockParam[] = await Promise.all(
              toolUseBlocks.map(async (call) => {
                ...
                try {
                  ...
                  return { type: "tool_result" as const, tool_use_id: call.id, content: JSON.stringify({ success: false, error: (error as Error).message }) };
                }
              })
            );
